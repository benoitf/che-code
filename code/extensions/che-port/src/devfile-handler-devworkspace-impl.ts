...
export class DevWorkspaceDevfileHandlerImpl implements DevfileHandler {
  async getEndpoints(): Promise<Array<Endpoint>> {
    ...
    console.log('received the extension', extension);
    const cheApi = extension?.exports;
    console.log('received the cheApi', cheApi);

    const outputChannel = vscode.window.createOutputChannel('Ports Plug-in 2');
    outputChannel.appendLine('received the extension' + JSON.stringify(extension));
    outputChannel.appendLine('received the isActive' + extension.isActive);
    outputChannel.appendLine('received the cheApi' + JSON.stringify(cheApi));
